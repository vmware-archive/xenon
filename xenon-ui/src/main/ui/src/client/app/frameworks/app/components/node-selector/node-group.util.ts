...
export class NodeGroupUtil {
     ...
     */
    private static _makeCircleOneNode(nodes: Node[], node0: Node): NodeGroupCircle {
        ...
        for (let i: number = 0; i < nodes.length; i++) {
            ...
            if (!this._isInCircle(circle, node1)) {
                if (circle.r === 0) {
                    circle = this._makeDiameter(node0, node1);
                } else {
                    circle = this._makeCircleTwoNodes(nodes.slice(0, i + 1), node0, node1);
                }
            }
        }
     ...
     */
    private static _makeCircleTwoNodes(nodes: Node[], node0: Node, node1: Node): NodeGroupCircle {
        var temp: NodeGroupCircle = this._makeDiameter(node0, node1);
        var containsAll: boolean = true;

        for (let i: number = 0; i < nodes.length; i++) {
            containsAll = containsAll && this._isInCircle(temp, nodes[i]);
        }
        ...
        for (let i: number = 0; i < nodes.length; i++) {
            ...
            let cross: number = this._crossProduct(node0.x, node0.y, node1.x, node1.y, node2.x, node2.y);
            let circle: NodeGroupCircle = this._makeCircumCircle(node0, node1, node2);

            if (circle === null) {
                continue;
            } else if (cross > 0 && (left === null ||
                this._crossProduct(node0.x, node0.y, node1.x, node1.y, circle.x, circle.y) >
                this._crossProduct(node0.x, node0.y, node1.x, node1.y, left.x, left.y))) {
                left = circle;
            } else if (cross < 0 && (right === null ||
                this._crossProduct(node0.x, node0.y, node1.x, node1.y, circle.x, circle.y) <
                this._crossProduct(node0.x, node0.y, node1.x, node1.y, right.x, right.y))) {
                right = circle;
            }
        }
    ...
    private static _makeCircumCircle(node0: Node, node1: Node, node2: Node): NodeGroupCircle {
        ...
        return {
            x: x,
            y: y,
            r: this._distance(x, y, x0, y0)
        };
    }

    private static _makeDiameter(node0: Node, node1: Node): NodeGroupCircle {
        return {
            x: (node0.x + node1.x) / 2,
            y: (node0.y + node1.y) / 2,
            r: this._distance(node0.x, node0.y, node1.x, node1.y) / 2
        };
    }

    private static _isInCircle(circle: NodeGroupCircle, node: Node): boolean {
        return circle !== null &&
            this._distance(node.x, node.y, circle.x, circle.y) < circle.r + EPSILON;
    }

    /**
     * Returns twice the signed area of the triangle defined by (x0, y0), (x1, y1), (x2, y2)
     */
    private static _crossProduct(x0: number, y0: number,
            x1: number, y1: number, x2: number, y2: number): number {
        return (x1 - x0) * (y2 - y0) - (y1 - y0) * (x2 - x0);
    }

    private static _distance(x0: number, y0: number, x1: number, y1: number): number {
