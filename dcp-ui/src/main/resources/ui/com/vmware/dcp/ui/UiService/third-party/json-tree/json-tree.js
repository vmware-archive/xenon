                ...
                '<ul ng-hide="node.isCollapsed">' +
                    '<li ng-repeat="key in utils.keys(json) track by key">' +
                        '<div draggable>' +
                            ...
                            '</span>' +
                            '<json-tree json="json[key]" edit-level="{{editLevel}}" collapsed-level="{{+collapsedLevel - 1}}" json-Keys="jsonKeys" node="childs[key]" timeout="{{timeout}}" ng-show="childs[key].isObject()"></json-tree>' +
                            '<span class="remove" ng-dblclick="utils.removeNode(key)" ng-show="node.isHighEditLevel"><span class="glyphicon glyphicon-minus glyph-red"></span></span>' +
                            '<span class="comma" ng-hide="utils.wrap.isLastIndex(node, $index + 1)">,</span>' +
                        '</div>' +
                    '</li>' +
                '</ul>' +
                '<span ng-bind="utils.wrap.end(node)"></span>' +
                '<span class="add" ng-show="node.isHighEditLevel && node.isObject()" ng-click="addTpl = !addTpl; jKey = null; inputValue = null"> <span class="glyphicon glyphicon-plus glyph-green"></span></span>' +
                '<span ng-show="(addTpl && node.isHighEditLevel) || false">' +
                    '<span ng-show="node.type() === \'object\'">' +
                        '<select ng-show="jsonKeys" ng-model="jKey" ng-options="k as k for k in jsonKeys" class="select-style"></select>' +
                        '<input type="text" ng-hide="jsonKeys" ng-model="inputKey" placeholder="key"/>: <input type="text" ng-model="inputValue" placeholder="value"/></span>' +
                    '<span ng-show="node.type() === \'array\'"><input type="text" ng-model="inputValue" placeholder="value"/></span>' +
                    '<button ng-click="utils.addNode(jKey, inputValue); addTpl = false"><span class="glyphicon glyphicon-ok glyph-blue"></span></button><button ng-click="addTpl = false"><span class="glyphicon glyphicon-remove glyph-red"></span></button>' +
                '</span>';
